...
; window.oMessenger = (function ($) {
    ...
    oMessenger.prototype.playSound = function (sFile, bRepeat = false) {
        ...
        try {
            if (bRepeat)
                this.oActiveAudioInstance = createjs.Sound.play(sFile, {
    ...
    oMessenger.prototype.initScrollArea = function () {
        ...
        $(talkBlock).scroll(function (e) {
            ...
            } else if (iPrevScrollHeight !== $(this).prop('scrollHeight')) {
    ...
    oMessenger.prototype.initFilesUploader = function (sSelector = '') {
        ...
        this.oFilesUploader = new oMessengerUploader({
            ...
            onUploadingComplete: (sName, aFiles, fCallback) => {
                ...
                const fUploadWhenReady = () => {
                    const iJotId = +(this.aUploaderQueue[sName]?.uploading_jot_id || 0);
                    if (iJotId) {
                        return fUpload(iJotId);
                    }

                    if (oThread?.promise) {
                        oThread.promise.then(() => {
                            const iReadyJotId = +(this.aUploaderQueue[sName]?.uploading_jot_id || 0);
                            if (iReadyJotId)
                                fUpload(iReadyJotId);
                            else {
                                if (typeof fCallback === 'function') fCallback();
                    ...
                    let iAttempts = 0;
                    const iMaxAttempts = 5;
                    const iDelayMs = 500;
                    const iWaiter = setInterval(() => {
                        iAttempts++;
                        const iPendingJotId = +(this.aUploaderQueue[sName]?.uploading_jot_id || 0);
                        const oPendingThread = this.oSendPool.get(id);
                        if (iPendingJotId) {
                            clearInterval(iWaiter);
                            fUpload(iPendingJotId);
                            return;
                        }

                        if (oPendingThread?.promise) {
                            clearInterval(iWaiter);
                            oPendingThread.promise.then(() => {
                                const iResolvedJotId = +(this.aUploaderQueue[sName]?.uploading_jot_id || 0);
                                if (iResolvedJotId)
                                    fUpload(iResolvedJotId);
                                else {
                                    if (typeof fCallback === 'function') fCallback();
                                    delete this.aUploaderQueue[sName];
                                }
    ...
    oMessenger.prototype.setUsersStatuses = function (oObject) {
        ...
        if (parseInt(iUser)) {
            ...
            if (typeof classList[2] !== 'undefined') {
                $('div[data-user-status="' + iUser + '"]').removeClass('online offline away').addClass(classList[2]).attr('title', _t('_bx_messenger_' + classList[2]));
            }
        }
    };
    ...
    oMessenger.prototype.sendMessage = function (sMessage, mixedObjects, fCallBack) {
        const _this = this,
            ...
            { threadReplies } = window.oMessengerSelectors.THREAD,
            {
                ...
                jotTitle,
                jotAvatar
            } = window.oMessengerSelectors.JOT;
            ...
            function ({ html, code, muted, params }) {
                ...
                if (typeof params !== 'undefined' && lot !== _this.oSettings.lot) {
                    ...
                    if (type !== area_type && area_type !== 'inbox')
                        return fBeep(muted);
                    else if (area_type === 'groups' && type === 'groups' && +group_id !== +_this.oSettings.group_id)
                        return fBeep(muted);
                }
                                ...
                                .waitForImages(oParent => {
                                    ...
                                    if (+iId)
                                        setTimeout(() => {
                                            ...
                                            _this.updateScrollPosition('top', 'fast', {
                                                pos: iTop,
                                                object: oObjects.first()
                                            });
                                        }, 0);
                                });
                            break;
    ...
    oMessenger.prototype.startVideoCall = function (oEl, iLotId, sRoom, oOptions = {}) {
        const _this = this,
            ...
            fDesktopCall = () => {
                let oParams = Object.assign({}, oOptions, !iLotId && {
                    url: encodeURIComponent(_this.oSettings.url),
                    title: _this.oSettings.title
                });
    ...
    return {
        ...
        init: function (oOptions) {
            ...
            _oMessenger.oHistory.pushState({
                action: 'init',
                lot: oOptions.lot,
                jot: oOptions.jot_id,
                area: oOptions.area_type
            }, null);
                        ...
                        .done(() => {
                            if (typeof _oMessenger.oHistory.pushState === 'function')
                                _oMessenger.oHistory.pushState({
                                    action: 'load_talk',
                                    lot: iLotId,
                                    area: _oMessenger.oSettings.area_type,
                                    jot: iJotId
                                }, null);
                ...
                .done(
                    function (oData) {
                        ...
                        if (!parseInt(oData.code)) {
                            ...
                            _oMessenger.sendMessage(sMessage, {
                                files: [{
                                    complete: 1,
                                    realname: fileName,
                                    name: fileName
            ...
            const _this = _oMessenger,
                ...
                fExecute = () => {
                    ...
                    $.post('modules/?r=messenger/get_users_list', {
                        group_id, area_type, lot, term: sTerm, except: function () {
                            let aUsers = [];
            ...
            const { group } = mixedGroup,
                {
                    ...
                    searchInput,
                    inboxAreaTitle
                } = window.oMessengerSelectors.TALKS_LIST;
        ...
        onReplyInThread: function (oObject) {
            ...
            _oMessenger.loadTalksListByParam(oParams, () => {
                ...
                _oMessenger.oHistory.pushState({
                    ...
                    lot: _oMessenger.oSettings.lot,
                    jot: +$(oObject).closest(jotMain).data('id')
                }, null);
            });

    ...
    */
    ...
    public function findThePageOwner($mixedPage)
    {
        ...
        if (isset($aTable['TriggerTable']) && isset($aTable['TriggerFieldId']) && isset($aTable['TriggerFieldAuthor']))
            return $this->getOne(
                "SELECT `{$aTable['TriggerFieldAuthor']}` 
    ...
    public function createNewLot($iProfileId, $aData, $aParticipants = array())
    {
        ...
        $sQuery = $this->prepare(
            "INSERT INTO `{$this->CNF['TABLE_ENTRIES']}` 
    ...
    function createJVC($iLotId, $iProfileId)
    {
        ...
        if (empty($aJVC)) {
            ...
            $this->query(
                "INSERT INTO `{$this->CNF['TABLE_JVC']}` 
                SET 
    ...
    {
        $this->query(
            "INSERT INTO `{$this->CNF['TABLE_JVCT']}` SET 
    ...
    {
        return $this->query(
            "UPDATE `{$this->CNF['TABLE_JVCT']}` SET 
                            `{$sField}`=:value                                                       
    ...
    function stopJVC($iLotId, $iProfileId)
    {
        ...
        $aTrack = $this->getRow(
            "SELECT * FROM `{$this->CNF['TABLE_JVCT']}`                                                                                           
                                        WHERE `{$this->CNF['FJVCT_ID']}`= :id",
            array(
                'id' => $aJVC[$this->CNF['FJVC_ACTIVE']]
            )
    ...
    {
        return $this->getRow(
            "SELECT * FROM `{$this->CNF['TABLE_JVC']}`
                                     WHERE `{$this->CNF['FJVC_LOT_ID']}`= :lot_id",
            array('lot_id' => $iLotId)
        );
    }
    ...
    public function markNotificationAsRead($iRecipientId, $iLotId)
    {
        ...
        foreach ($aEvents as &$aEvent) {
            if (
                $this->query(
                    "DELETE FROM `bx_notifications_events` WHERE `id`=:id",
                    array(
                        'id' => $aEvent['id']
                    )
                )
    ...
    function updatePublicVideoRoom($sRoom, $iProfileId, $sAction)
    {
        ...
        return $this->query(
            "UPDATE `{$this->CNF['TABLE_PUBLIC_JVC']}` 
                                    SET
    ...
    function updateLotAttachmentType(&$aData)
    {
        ...
        return $this->query(
            "REPLACE INTO `{$this->CNF['TABLE_LOT_ATTACHMENTS']}` 
                                            SET 
    ...
    function addGroup($iProfileId, $aParams)
    {
        ...
        if (empty($aGroup))
            return $this->query(
                "INSERT INTO `{$this->CNF['TABLE_GROUPS']}` 
                ...
                array(
                    'name' => $aParams[$this->CNF['FMG_NAME']],
                    'author' => $iProfileId,
    ...
    {
        return $this->getOne(
            "SELECT COUNT(*) FROM `{$this->CNF['TABLE_SAVED_JOTS']}` 
                                               WHERE  `{$this->CNF['FSJ_ID']}`=:jot_id AND `{$this->CNF['FSJ_PROFILE_ID']}`=:profile_id",
            ['jot_id' => $iJotId, 'profile_id' => $iProfileId]
        ) != 0;
    }
    ...
    function deleteSavedJotItems($iJotId = 0, $iProfileId = 0)
    {
        if ($iJotId && $iProfileId)
            return $this->query(
                "DELETE FROM `{$this->CNF['TABLE_SAVED_JOTS']}` 
                                           WHERE `{$this->CNF['FSJ_ID']}`=:jot_id AND `{$this->CNF['FSJ_PROFILE_ID']}`=:profile_id",
        ...
        return $iProfileId ? $this->query(
            "DELETE FROM `{$this->CNF['TABLE_SAVED_JOTS']}` 
                                           WHERE `{$this->CNF['FSJ_PROFILE_ID']}`=:profile_id",
            array('profile_id' => $iProfileId)
        ) : false;
    }
    ...
    {
        return $this->getOne(
            "SELECT COUNT(*) FROM `{$this->CNF['TABLE_JOTS_MEDIA_TRACKER']}`					
    ...
    private function getProfilesByCriteriaSqlParts($aFields)
    {
    ...
    public function getProfilesByCriteria($aFields, $aAddons = [])
    {
        return $this->_getProfilesByCriteria($aFields, $aAddons, false);
    }

    public function getProfilesCountByCriteria($aFields, $aAddons = [])
    {
        return $this->_getProfilesByCriteria($aFields, $aAddons, true);
    }

    private function _getProfilesByCriteria($aFields, $aAddons = [], $bCountOnly = false)
    {
        if (empty($aFields) && empty($aAddons)) {
            return $bCountOnly ? 0 : [];
        }

        $aExcludeProfiles = $aAddons['exclude_profiles'] ?? [];
        $aIncludeProfiles = $aAddons['include_profiles'] ?? [];

        $aQueries = [];
        if (!empty($aFields) || empty($aIncludeProfiles)) {
            $aSqlParts = $this->getProfilesByCriteriaSqlParts($aFields);
            $sWhereClause = !empty($aSqlParts['where']) ? $aSqlParts['where'] : '';

            if (!empty($aExcludeProfiles)) {
                $sWhereClause .= " AND `sys_profiles`.`id` NOT IN (" . implode(',', $aExcludeProfiles) . ")";
            }

            $aQueries[] = "SELECT DISTINCT `sys_profiles`.`id` FROM `sys_profiles`" . $aSqlParts['join'] . " WHERE 1" . $sWhereClause;
        }

        if (!empty($aIncludeProfiles)) {
            $sExcludeClause = '';
            if (!empty($aExcludeProfiles)) {
                $sExcludeClause = " AND `sys_profiles`.`id` NOT IN (" . implode(',', $aExcludeProfiles) . ")";
            }
            $sQueriesParts = implode(',', $aIncludeProfiles);
            $aQueries[] = "SELECT `sys_profiles`.`id` FROM `sys_profiles` WHERE `sys_profiles`.`id` IN ({$sQueriesParts})" . $sExcludeClause;
        }

        $sSql = implode(" UNION ", $aQueries);

        if ($bCountOnly) {
            $sSql = "SELECT COUNT(*) FROM ({$sSql}) AS `t`";
            return (int)$this->getOne($this->prepare($sSql));
        } else {
            return $this->getColumn($this->prepare($sSql));
        }
    }
